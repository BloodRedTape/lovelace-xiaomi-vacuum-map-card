...
import { ActionHandlerEvent, fireEvent, forwardHaptic, LovelaceCard, LovelaceCardEditor } from "custom-card-helpers";
...
export class XiaomiVacuumMapCard extends LitElement implements Omit<LovelaceCardEditor, "hass"> {
    ...
    @state() private furnitureEditMode = false;
            ...
            <div
                ...
                <div id="map-image-overlay">
                    <svg
                        ...
                        ${this._drawFurniture()}
                        ${validCalibration && !this.furnitureEditMode ? this._drawSelection() : null}
                    </svg>
                </div>
            </div>
        `;
                ...
                <div class="map-wrapper">
                    ...
                    <div id="map-zoomer-overlay">
                        <div style="right: 0; top: 0; position: absolute;">
                            ...
                            ${conditional(this._isInEditor(), () => html`
                                <ha-icon
                                    icon="${this.furnitureEditMode ? "mdi:content-save-edit-outline" : "mdi:home-edit"}"
                                    class="standalone-icon-on-map clickable ripple"
                                    @click="${this._toggleEditMode}">
                                </ha-icon>
                                ${conditional(this.furnitureEditMode, () => (this.config.furniture_types ?? []).map(type => html`
                                    <ha-icon
                                        icon=${type.icon}
                                        class="standalone-icon-on-map clickable ripple"
                                        @click="${() => this._addFurniture(type.url)}"></ha-icon>`
                                ))}`
                            )}
                        </div>
    ...
    private _drawFurniture(): SVGTemplateResult | null {
        return svg`${this.furniture.map(p => p.render())}`;
    }
    ...
    private _toggleEditMode(): void{
        if(this.furnitureEditMode)
            this._saveFurniture();
        this.furnitureEditMode = !this.furnitureEditMode;

        this.requestUpdate();
    }

    private _addFurniture(url: string): void {
        const preset = this._getCurrentPreset();
        const currentMode = this._getCurrentMode();
        const marginTop = preset.map_source.crop?.top ?? 0;
        const marginBottom = preset.map_source.crop?.bottom ?? 0;
        ...
        const width = actualWidth / 3 / this.mapScale;
        const height = actualHeight / 3 / this.mapScale;
        this.furniture.push(new Furniture(x, y, width, height, 0, url, this._getContext()));
        this.requestUpdate();
    }

    private _saveFurniture(): void{
        if (this._isInEditor()) {
            const event = new Event(EVENT_FURNITURE_CHANGED);
            (event as any).furniture = this.furniture.map(f => f.getConfig());
            window.dispatchEvent(event);
        }
    }

    private _ensureFurniture(): void{
        if (this.furniture.length)
            return;

        this.furniture = (this.config.furniture ?? []).map(config => Furniture.fromConfig(config, this._getContext()))
    }
